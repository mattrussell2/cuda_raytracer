...
{
	//ppms.push_back(new ppm("./data/image/bark0.ppm"));
	//ppms.push_back(new ppm("./data/image/bark1.ppm"));
	//ppms.push_back(new ppm("./data/image/bark2.ppm"));
	//ppms.push_back(new ppm("./data/image/grnd3.ppm"));	
	file_name = name;
...
bool SceneParser::parse() 
{
   ...
   if (!txn->ValueStr().compare("scenefile"))
   {
	  ...
	  //flatten light objects into an array
	  numLights = m_lights.size();
	  lightList = (SceneLightData *)malloc(sizeof(SceneLightData) * numLights);
	  for (int i = 0;i < m_lights.size();i++) {
		  lightList[i] = *m_lights.at(i);		
	  }
...
bool parseMap(const TiXmlElement* e, SceneFileMap* map)
{
   ...
   //printf("filename is: %s", map->filename);

  // map->ppm_obj = new ppm(map->filename);
...
bool SceneParser::parsePrimitive(const TiXmlElement* prim, SceneNode* node)
{
   ...
   else if (!primType.compare("pyramid"))
	   primitive->type = SHAPE_PYRAMID;
   else if (!primType.compare("taper"))
	   primitive->type = SHAPE_TAPER;
   ...
   while(primdata)
   {
      ...
      {	
		 //TEXTURE MAP INSTANTIATION
		 //filanmes are: './data/images/bark0.ppm'
		 string filename = primdata->FirstAttribute()->ValueStr();	
		// printf("filename: %s", filename.c_str());
		 mat.textureMap->ppm_obj = ppms.at(filename.c_str()[17] - '0');//filename[34] - '0'); // fix later
		 mat.textureNum = filename.c_str()[17] - '0';
															   //printf("text num: %i", filename[17] - '0');
		// getchar();
		// exit(1);
